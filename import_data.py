...
async def import_matches(db):
    """Import all match JSON files."""
    matches_table = await db.create(Match, pk="id", if_not_exists=True,
                                    indexes=["match_type", "gender", "season",
                                             "team1", "team2", "event_name"])
    dates_table = await db.create(MatchDate, pk="id", if_not_exists=True,
                                  indexes=["match_id"])
    players_table = await db.create(MatchPlayer, pk="id", if_not_exists=True,
                                    indexes=["match_id", "person_id"])
    innings_table = await db.create(Innings, pk="id", if_not_exists=True,
                                    indexes=["match_id"])
    deliveries_table = await db.create(Delivery, pk="id", if_not_exists=True,
                                       indexes=["innings_id", "batter_id",
                                                 "bowler_id"])
    wickets_table = await db.create(Wicket, pk="id", if_not_exists=True,
    ...
    for i, filepath in enumerate(all_files):
        ...
        for inn_num, inn_data in enumerate(data.get("innings", [])):
            ...
            # Collect all deliveries for this innings
            delivery_rows = []
            wicket_pending = []  # (index_in_delivery_rows, wicket_data)
                ...
                for del_idx, del_data in enumerate(over_data["deliveries"]):
                    ...
                    if "wickets" in del_data:
                        for w in del_data["wickets"]:
                            wicket_pending.append((
                                len(delivery_rows) - 1, w
                            ))

            # Bulk insert deliveries, get IDs back for wickets
            if delivery_rows:
