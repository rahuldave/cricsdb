...
- **Scope-link architecture** (shipped 2026-04-19). All player/team name rendering goes through `PlayerLink` / `TeamLink` (in `components/`) following one model: **plain name link = all-time + gender only; `(e, t)` / `(s, b)` letter links = scoped variants** that drop only their axis. Table-driven via `components/scopeLinks.ts`:
  - `FILTER_KEYS` is the single source of truth for which FilterBar fields participate. Adding a new filter is ONE edit — `useFilters` (in `hooks/`), FilterBar's dropdown-narrowing fetch, scope-link URL building, and `useFilterDeps` all iterate this list.
  - `TIER_SPECS` declares per-letter `drops`; (e) no drops, (t) drops season, (s) no drops, (b) drops tournament+season. Everything else rides through.
  - `ScopeContext` promotes path-identity into filter pinnings for `/teams?team=X`, `/venues?venue=X`, and `/head-to-head?mode=team&team1=A&team2=B` — one `<ScopeContext.Provider>` per page subtree, merged on top of `useFilters` via `useScopeFilters`.
  - Layering: `FilterBar state → ScopeContext → SubscriptSource (per-row override)`, each later layer overriding the earlier.
  - Tooltips on each letter spell out the destination scope ("Indian Premier League (2024) at Wankhede Stadium stats") so the convention is learnable without docs.
- **FilterBar dropdown narrowing respects every FilterBar field** (shipped 2026-04-19). `getTournaments` and `getSeasons` receive the full FilterParams payload plus page-local `series_type`; backend `/api/v1/tournaments` + `/api/v1/seasons` accept and narrow by `season_from` / `season_to` / `filter_venue` / `series_type` via the same `_reference_clauses` helper. Effect: on `/series?series_type=bilateral&filter_team=India&filter_opponent=Australia`, the tournament dropdown hides T20 World Cup and shows only bilateral series between the two teams. Regression harness at `tests/regression/filterbar_refs/` pins 14 REG byte-identical + 7 NEW differs. Integration tests in `tests/integration/series.sh` Tests 5a/5b.
- **Venue canonical names never carry a trailing `, <City>` suffix** (shipped 2026-04-18). 198 venues were Phase-1-catalogued as "X, <City>" with the city column also holding "<City>" — redundant duplication in the UI. `scripts/strip_venue_suffix.py` rewrote `api/venue_aliases.py` once; `scripts/fix_venue_names.py` retrofits the DB. Parens-style disambiguators (`County Ground (Taunton)` × 6, `National Stadium (Karachi)/(Hamilton)`, etc.) are a different pattern and stay. **For new/unknown venues: `resolve_or_raw` applies a `_strip_city_suffix` fallback on dict miss** so a brand-new cricsheet venue like `"Narendra Modi Stadium II, Ahmedabad"` with city=`"Ahmedabad"` auto-resolves to `("Narendra Modi Stadium II", "Ahmedabad", None)` without waiting for a worklist round-trip. `update_recent.py` reuses `import_data.import_match_file`, so the same hook fires on full rebuild AND incremental paths — one function, both codepaths. Country still backfills via the unknowns-log → worklist review cycle.
...
- **Linking + navigation audit (2026-04-18).** Page-by-page walk of every top-level route against the "match-list date convention", "two-link name + context pattern", and "links up / dead ends" rules in this file. 21 findings seeded as a survey, then worked through tab-by-tab in the same session (see below).
...
**Shipped 2026-04-18 (same session):**
...
- Two-architectural follow-ups from 2026-04-18 audit survive unchanged: **`TeamLink` component** (name → overall + context suffix for tournament+season or bilateral+season) and **scorecard linkability API batch** (`player_of_match`, dismissal text, did-not-bat, fall-of-wickets need `PersonRef` shapes). See `internal_docs/design-decisions.md`.

**NEXT SESSION agenda (in order):**

1. **Venue-cell sweep** (deferred from the 2026-04-18 audit) — one sweep across the remaining plain-text venue cells: Series Knockouts + Matches (items 15, 10 in audit), HeadToHead by-match venue (#8 partial), anywhere else venue text still appears plain. Link them all to `/venues?venue={venue}` (now a real dossier) — or `?filter_venue={venue}` where the source context wants a filter rather than a drilldown.
2. **`TeamLink` component + cross-site disambiguation** (punted from 2026-04-18 audit; see `internal_docs/design-decisions.md` "Team-name link scope ambiguity"). Name → overall; context suffix → tournament+season or bilateral+season scoped. Site-wide replacement of the existing inline `teamHref` helpers. Medium-sized refactor.
3. **Scorecard linkability API batch** (deferred from 2026-04-18 audit). Change `/api/v1/matches/{match_id}` response shapes for `player_of_match`, dismissal text, did-not-bat, fall-of-wickets to return `PersonRef`s; frontend wraps each in a compact `PlayerLink`. See `internal_docs/design-decisions.md` for the full shopping list.
4. **filterDeps refactor (Option B)** — partially done. `useFilterDeps()` (in `hooks/useFilterDeps.ts`) returns `FILTER_KEYS.map(k => filters[k])`, stable-memoized. Pages can gradually adopt it to replace their hand-rolled arrays. The underlying single-source-of-truth registry (`FILTER_KEYS`) lives in `components/scopeLinks.ts` and is now shared by `useFilters`, FilterBar's URL reads + dropdown-narrowing fetch, scope-link URL building, and `useFilterDeps`. See `internal_docs/design-decisions.md` "filterDeps arrays — explicit, per-page, easy to under-wire (revisit)".
