...
1. **Venues Phase 3** — per-venue dossier at `/venues?venue=X`. Tabs: Overview (avg 1st-inn total, bat-first win %, toss-decision pie, boundary % by phase, highest/lowest totals, matches-hosted-by-tournament×gender×season) + Batters/Bowlers/Fielders leaders + Matches + Records. One new endpoint needed (`GET /api/v1/venues/{venue}/summary`) — all other dossier tabs reuse existing `/api/v1/{batters,bowlers,fielders}/leaders?filter_venue=X` and `/api/v1/matches?filter_venue=X` for free. See spec `internal_docs/spec-venues.md`.
2. **Venue-cell sweep** (deferred from the 2026-04-18 audit) — after Venues Phase 3 lands, do one sweep across the remaining plain-text venue cells: Series Knockouts + Matches (items 15, 10 in audit), HeadToHead by-match venue (#8 partial), anywhere else venue text still appears plain. Should all link `?filter_venue={venue}` or (once Phase 3 exists) `/venues?venue={venue}` for a dossier arrival.
3. **`TeamLink` component + cross-site disambiguation** (punted from 2026-04-18 audit; see `internal_docs/design-decisions.md` "Team-name link scope ambiguity"). Name → overall; context suffix → tournament+season or bilateral+season scoped. Site-wide replacement of the existing inline `teamHref` helpers. Medium-sized refactor.
4. **Scorecard linkability API batch** (deferred from 2026-04-18 audit). Change `/api/v1/matches/{match_id}` response shapes for `player_of_match`, dismissal text, did-not-bat, fall-of-wickets to return `PersonRef`s; frontend wraps each in a compact `PlayerLink`. See `internal_docs/design-decisions.md` for the full shopping list.
5. **filterDeps refactor (Option B)** — derive per-page deps from `Object.values(filters)` so new filters auto-wire. See `internal_docs/design-decisions.md` "filterDeps arrays — explicit, per-page, easy to under-wire (revisit)".
