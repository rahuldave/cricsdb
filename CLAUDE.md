...
- `frontend/src/components/{InningToggle,teams/InningBandsRow}.tsx` — page-local 1st/2nd-innings UI (spec-inning-split.md). `InningToggle` is a three-pill segmented control writing URL `?inning=`; mounted on player Batting/Bowling/Fielding pages and team Batting/Bowling/Fielding/Partnerships/BySeason/vsOpponent tabs. `InningBandsRow` is the compact 3-row band table (Overall / 1st innings / 2nd innings) under each team-side discipline tab, parallel to `PhaseBandsRow.tsx` but on `i.innings_number` instead of over phases. Reads `/teams/{team}/{batting,bowling,fielding,partnerships}/by-inning` for the inning rows; the Overall row reads from the same `/summary` endpoint the page already fetches.
...
- **FilterBarParams + AuxParams split.** `api/filters.py` separates 10 FilterBar UI fields (`FilterBarParams`, mirrors frontend `FILTER_KEYS`) from internal-plumbing narrowings (`AuxParams`, currently holds `scope_to_team` for the Compare-tab avg slot's auto-narrow, `chip_team_class` (DEPRECATED 2026-04-29) and `chip_baseline_scope_json` (the general chip-baseline alignment mechanism — base64-JSON of the avg slot's full effective scope; `_league_aux` decodes it and uses it as the league-side filters for chip envelope `scope_avg`)). `series_type` lived in AuxParams until 2026-04-28; promoted to FilterBarParams as the 10th UI field (spec `internal_docs/spec-filterbar-series-type.md`). Routers declare `aux: AuxParams = Depends()` alongside `filters: FilterBarParams = Depends()` and call `filters.build(aux=aux)`. `FilterParams` preserved as alias. Future page-local filters (result_filter, close_match, toss_decision) belong in `AuxParams`. **When adding a shared helper that calls `filters.build()`, it MUST take `aux: AuxParams | None = None` as a parameter and pass it through.** Omitting it leaves `aux` as a free variable — import-time silent, request-time 500. Grep `filters\.build\(` in `api/` before shipping a helper refactor; every call site should have `aux=aux` or `aux=None`. This is how the Teams > Partnerships 500 (2026-04-20) lurked across multiple deploys.

- **`__any__` slot-override sentinel.** Per-slot URL params on the Compare tab (`compareN_<axis>=…`) accept the literal string `__any__` as "explicit empty / do not inherit primary." Used when the user wants to broaden a slot past primary's narrowing — e.g. primary `season=2025` with `compare1_season_from=__any__&compare1_season_to=__any__` resolves the avg slot to all-time. Backend treats `__any__` as no-narrowing in every clause guard via `api/filters._is_set()` (also applied in hand-rolled helpers in `tournaments.py::_build_filter_clauses`, `reference.py::_reference_clauses` + `list_teams` + `search_players`, `bucket_baseline_dispatch`). Frontend `useUrlState`'s URL-delete-on-falsy doesn't fire for `__any__` (truthy), so the sentinel persists in the URL. `useCompareSlots.readSlot` decodes it: still records the override (so `SlotHeaderChip` surfaces "any season" / "all teams"), but resolves `scope[axis]=undefined` so the slot's fetcher sends no narrowing. Spec: `internal_docs/spec-slot-override-chip-alignment.md`.

- **Chip-baseline alignment (`chip_baseline_scope_json`).** When a Compare-tab team slot has a peer avg slot, the team-side chip envelopes' `scope_avg` must numerically equal what the avg COLUMN displays for the same metric. `TeamCompareGrid.chipAlignmentFor` serializes the avg slot's full effective scope (FilterBar fields + synthesized `scope_to_team` when the auto-narrow applies) as base64-JSON in `chip_baseline_scope_json`. Backend `_league_aux` decodes it (Path 0, priority over the legacy `scope_to_team` synthesis + `chip_team_class` shortcut paths) and returns the decoded scope as the league-side filters. The team-side aggregation stays scoped to the team's own filters (Aus shows all 22 of its matches), but the chip's `scope_avg` is computed against the avg slot's pool. This generalises the v3 narrow-direction `chip_team_class` hint to handle BROADENING + override-to-empty on every overridable axis. `chip_team_class` kept as back-compat shortcut through the soak window; deprecation note in `AuxParams.chip_team_class` description lists removal steps. Math invariant `chip.scope_avg == avg.displayed` enforced by `tests/sanity/test_chip_direction_invariant.py` (no-override case) + `tests/sanity/test_slot_override_alignment.py` (broaden + override-to-empty cases) + `tests/integration/dom/cross_cutting_slot_override_chip_align.sh` (DOM-level). **When touching `_league_aux` or per-innings transform call sites, the divisor's filters must come from `league_filters` not the team's primary `filters`** — surfaced 2026-04-29 (partnerships divisor was passing the wrong filters; latent bug under the legacy chip_team_class path, real bug under chip_baseline_scope_json). Spec: `internal_docs/spec-slot-override-chip-alignment.md`.
