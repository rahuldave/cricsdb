...
**NEXT SESSION: implement R (Players tab)** — spec v2 finalised in **`internal_docs/spec-players.md`** on 2026-04-16, ready to build. Summary: new `/players` route = person-focused overview stacking batting / bowling / fielding / keeping summary rows, plus N-way compare mode (`compare=Y,Z`, cap at 3 players). Nav restructure: Batting/Bowling/Fielding collapse into a `Players ▾` dropdown (desktop) + a mobile sub-row, so the three discipline routes live *under* Players in the nav while staying reachable at their existing URLs. Home-page PlayerLinks change to point at `/players?player=X` with a tiny `b · bw · f` italic suffix linking to the discipline deep-dives. Curated popular-profiles + popular-compares tiles on the landing. All existing FilterBar filters flow through (all-time default — no useDefaultSeasonWindow). Auto-gender on pick via the existing self-correcting-deep-link pattern. Effort ~2 working days; near-zero backend (composes existing `/summary` endpoints client-side via `getPlayerProfile`). Start here.

**After R: O — Tournament-baseline comparison overlays** on team / batter / bowler / fielder pages. M shipped the per-tournament endpoints with explicit baseline reusability — call any `/api/v1/series/{summary,batters-leaders,…}` without a team filter to get the tournament-wide baseline, with one for the team's narrowed view (responses are shape-compatible). Frontend wiring needed: overlay league means on team-tab charts, add "vs league avg" columns to player tables, support "delta from league mean" colour mode on heatmaps. Design sketch in `internal_docs/design-decisions.md` "Team metrics need tournament baselines (revisit when /tournaments ships)".
