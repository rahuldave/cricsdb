...
**L. Fielding analytics page.** _Tier 1 done._ `/fielding` page with `fielding_credit` denormalized table (~118K rows), `fielder_aliases.py`, `wicket.fielders` double-encoding fix in `import_data.py`, 7 API endpoints (`api/routers/fielding.py`), frontend page with 6 tabs (By Season, By Over, By Phase, Dismissal Types, Victims, Innings List). Fielder search via `role=fielder` in `/api/v1/players`. Both `import_data.py` and `update_recent.py` auto-populate fielding credits — no separate script needed. Spec at `docs/spec-fielding.md`.
   - **Tier 2** (build-ready spec at `docs/spec-fielding-tier2.md`): wicketkeeper identification via 4-layer algorithm (stumping → season-candidate → career N≥3 → team-ever-keeper). New `keeper_assignment` table with `keeper_id` (nullable), `confidence` enum (`definitive/high/medium/low`), `method` tag, and `ambiguous_reason` for the 17.8% NULL rows. Expected coverage: 82.2% assigned. Ambiguous-match worklist exported to `docs/keeper-ambiguous.csv` for later Cricinfo/manual disambiguation. Used across fielding page ("Keeping" sub-tab), match scorecards (per-innings keeper label), and team pages (keepers used).
