...
**Teams → Compare tab** (`/teams?team=X&tab=Compare[&compare=Y[,Z]][&avg_slot=1]`): parallel to Players compare. Side-by-side column grid of up to 3 teams across **Results / Batting / Bowling / Fielding / Partnerships** rows, compact label/value layout, FlagBadge on each column (null-renders for franchise sides). Backed by a frontend composer `getTeamProfile(team, filters)` (`api.ts`) that parallel-fetches 12 endpoints per team: 5 summaries (`team_summary`, `team_batting/summary`, etc. plus the new `team_partnerships/summary`) + 6 deep-lens endpoints (`batting/by-phase`, `bowling/by-phase`, `partnerships/by-wicket`, `batting/by-season`, `bowling/by-season`, `fielding/by-season`, `partnerships/by-season`) so phase bands + partnership-by-wicket sub-rows + season trajectory render without a second round-trip. Cross-gender / cross-team_type adds are prevented upstream by the FilterBar auto-narrow (locks gender + team_type from the primary); the picker additionally rejects any candidate whose in-scope match count is zero. A self-correcting effect in `Teams.tsx` auto-switches `tab` to `Compare` with `{replace:true}` when a share URL arrives carrying `compare=` but no `tab=`, so share links work without the sender needing to copy the tab param too. See `internal_docs/design-decisions.md` "Team Compare" entry for the cross-type gating rationale (and a pre-existing `fielding/summary.matches` bug this surfaced).

**Compare tab — average-team column** (Spec 1 of `internal_docs/spec-team-compare-average.md`): URL param `avg_slot=1` adds a fourth column that is the pool-weighted league baseline scoped to the current FilterBar. Backed by `getScopeAverageProfile()` → `/api/v1/scope/averages/*` (12 mirrors of the team endpoints, helpers `_team_innings_clause` and `_partnership_filter` accept `team=None` to drop the team filter — same SQL surface, single source of truth). FlagBadge null-renders; the column header is scope-computed (e.g. "Indian Premier League 2024 avg" — `scopeAvgLabel` in `teamUtils.ts`); identity-row says "N matches in scope". The picker (`AddTeamComparePicker`) gains a "+ Add league average" button that flips to absent once added. Phase bands (PP/Mid/Death sub-rows under Batting+Bowling: `PhaseBandsRow.tsx`) and partnership-by-wicket sub-rows (1st-10th wicket: `PartnershipByWicketRows.tsx`) render in every column. Below the grid, `SeasonTrajectoryStrip.tsx` renders a 2-panel chart strip (Batting RR by season + Bowling Econ by season) when the scope spans ≥2 seasons; hidden under single-season filters. Small-sample suppression: `n < 30` partnerships at a wicket position render `—` in the average column only (team columns never suppress). Phase 2B (`api/metrics_metadata.py`) wraps each numeric metric on the 5 team-compare summary endpoints in `{value, scope_avg, delta_pct, direction, sample_size}` envelope — Spec-1 UI reads only `.value`; the rest is shipped for Spec-2 surfaces (player compare, leaderboard delta columns, H2H baseline — see `internal_docs/outlook-comparisons.md`). Identity-bearing nested objects (`highest_total`, `best_pair`, `keepers`, `gender_breakdown`) stay flat — they're not metrics. Per-match fielding rates' `scope_avg` is halved (each match has 2 fielding sides — team-side comparable is /2).
