...
**I. Responsive chart sizing.** Every chart component currently has hardcoded `width={600}` (or 700) props. On mobile the SVG would bust the layout, so chart cards now wrap in `overflow-x-auto` — the chart keeps its intrinsic size and the user scrolls horizontally inside the card. The proper fix is to drop the hardcoded width and have the chart fill its container, via either a `ResizeObserver` in the chart wrapper or Semiotic's responsive-frame helper if it exists. Worth doing because horizontal scroll inside a card is a flag for "I gave up" — the charts should *shrink*, not scroll.
