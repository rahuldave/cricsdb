...
### Shipped 2026-04-28 (team_class promoted to FilterBar — v3 spec)

`team_class=full_member` was a per-slot Compare-tab override on
2026-04-27 (Mechanism B above). User asked to promote it to a
proper FilterBar field so every tab respects it natively, not just
the Compare-tab avg slot.

**The corrected mental model.** v2 of the spec framed FilterBar
team_class as a new "symmetric narrowing mode (C)" that needed to
coexist with the per-slot mode (B) and a baseline mode (A) — three
modes needing parallel test anchors. Two audits on 2026-04-28
traced the consequences and showed v2 was wrong: there are no
modes, just one axis with a default-flow + per-slot override
capability — exactly the same shape as `tournament` / `season` /
`filter_venue`. v3 (`spec-filterbar-team-class-v3.md`) collapses
v2's ~50 pages into a one-line `useCompareSlots:50` bugfix plus a
small set of follow-on plumbing.

**5-commit rollout.**

1. **Backend field move.** `team_class` moves from `AuxParams` to
   `FilterBarParams` with a defensive intl-only gate inside
   `build()`. `_league_aux` refactored to `(filters, aux)` tuple
   so the chip alignment hint (`chip_team_class`) lands on a
   league-side filters copy instead of aux. 7 call sites
   destructured. `is_precomputed_scope` rejects on
   `filters.team_class` (gated). 3 sanity-test fixtures updated
   for the field move (bundled in commit 1 — they crash
   otherwise).
2. **Frontend FilterBar widget.** `FILTER_KEYS` extended to 9.
   Pill (intl-only, between Tournament and Seasons). Auto-clear
   effect on Type change. Status-strip chip. **The literal
   `useCompareSlots:50` bugfix:** `team_class: undefined` →
   `team_class: primary.team_class`. Compare-tab `primarySlotOf`
   was also missing `team_class` in its scope (caught during
   browser-verify — Aus showed 22 instead of 16 on URL E1).
3. **Hand-rolled filter helper fan-out.** `_build_filter_clauses`
   in `tournaments.py` (17 Series endpoints), `_reference_clauses`
   in `reference.py` (`/tournaments` + `/seasons` dropdowns),
   `list_teams` in `reference.py` (FilterBar typeahead) — each
   bypasses `filters.build()` so each gets a defensive-gated
   `full_member_clause` branch. Without commit 3, Series tab and
   the FilterBar dropdowns silently ignore the pill.
4. **Regression URL fan-out.** 121 NEW intl-FM URLs across 10
   suites (`teams` 32, `scope-averages` 14, `batting` 10,
   `bowling` 9, `fielding` 13, `players` 21, `head_to_head` 9,
   `matches` 3, `venues` 7, `filterbar_refs` 3). Generated by
   `tests/regression/team_class_url_gen.sh`. Each NEW mirrors a
   REG sibling with `&team_class=full_member` appended.
5. **Tests.** `test_team_class_baseline_numbers.py` populated
   with 30 anchor assertions (16 match counts, 4 chip baselines,
   2 women's symmetry, 4 club no-op gate, 4 top-10 leaderboard).
   `team_class_url_audit.py` fully implemented with
   capture-before-and-after diff + AFTER snapshot committed
   (392 URLs across 24 surfaces, 0 silent-ignore). 6 new
   integration scripts (filterbar / gating / persistence /
   per-tab-narrowing / compare E1 / compare club no-op). 3
   existing scripts updated (compare_filters Anchor 6,
   compare_avg_chips Anchor E1, cross_cutting_url_state Test 3).

**Outcome on the canonical URL E1**
(`?team=Australia&compare1=__avg__&compare2=India&gender=male&team_type=international&season_from=2024&season_to=2025&team_class=full_member&tab=Compare`):
- Australia col: **22 → 16** matches (FM-narrowed).
- Avg col: **870 → 140** matches; label changes to
  "Full-member average".
- India col: **34 → 31** matches.
- Chip alignment is native: both team-side and league-side
  baselines compute against `filters.team_class=fm` from the same
  source. No `chip_team_class` hint needed on inheritance path.

**Defensive gate on club URLs:** RCB IPL 2025 page with
`&team_class=full_member` strips the param via FilterBar's
auto-clear effect (replace mode, no history pollution). Backend
defensive gate makes `team_class=fm` a no-op for `team_type='club'`
in case a curl/share-link slips through (Bp2 = Bp1 = 15).

**Test coverage post-ship:**
- 4/4 sanity tests green (incl. 30 new anchor assertions).
- 11/11 regression suites: 0 REG drift across 264 REG URLs.
- 121 NEW intl-FM URLs added (will flip to REG after 1-2 weeks
  of stable HEAD per spec §9 optional commit 6).
- 6/6 v3 integration scripts green; 3/3 modified scripts green.
- URL audit: 392 URLs across 24 surfaces, 0 silent-ignore bugs.

**Followups parked as sibling specs (not blocking):**
- `spec-slot-override-chip-alignment.md` (NOT YET WRITTEN) —
  override-to-empty URL serialization + chip alignment under
  bidirectional override. Affects EVERY overridable axis
  (tournament / season / venue / series_type / team_class), not
  just team_class. The URL machinery has no way to express
  "override to (none)" while inheriting a non-empty primary —
  needs a sentinel value.
- `spec-filterbar-series-type.md` (NOT YET WRITTEN) — promote
  `series_type` from AuxParams to FilterBarParams the same way.
  Same shape as v3; ~250 NEW regression URLs and 22-surface
  matrix again. Doing both axes in a single session would 2× the
  mechanical work.
...
- **Compare-tab social meta — weak title + missing scope.** `api/social_meta.py:158-169` builds the Compare-URL Twitter card title as `"<team> vs avg team — Compare"` (or `"<team> vs <team2> vs avg team"`) and appends `scope_tag` (gender/season/tournament/full-member) only to the description, not the title. Two issues: (1) **"avg team" is generic** — for clubs the rendered column header is "Indian Premier League average" via `scopeAvgLabel` in `frontend/src/components/teams/teamUtils.ts`; the social-meta title should mirror that ("IPL avg" / "Aus T20I avg" / etc.) so a shared link doesn't read "Mumbai Indians vs avg team". (2) **Scope is dropped from the title** — the description gets `· men's · 2024-2025` but the card title (which is what social previews surface most prominently) doesn't, so a "MI vs avg team" preview reads as all-time when the URL is actually IPL 2025. Fix path: lift `scopeAvgLabel`'s logic into `social_meta.py` (it's pure-string from the URL params already in scope), and append a compact scope suffix to the Compare title (e.g. `"Mumbai Indians vs IPL avg — Compare · 2024-2025 men's · T20 & CricsDB"`). Not urgent — the page itself renders correctly; only social previews look thin. Same generic-baseline issue applies on team-rivalry and player-compare URLs but is less visible.
