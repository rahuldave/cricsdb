...
scripts/populate_player_scope_stats.py — Builds player_scope_stats table — denormalized per-(person, scope_key) aggregates. Consumed by the player-baseline rollout (spec-player-compare-average.md). Auto on import + update.
scripts/populate_playerscopestats_position.py — Per-(person, scope_key, position_bucket) batting aggregates. 10 buckets — opener=1+2 merged, then #3..#11. Backs the cohort baseline endpoint + the per-player position-distribution histogram on /batters/{id}/summary. Auto on import + update.
