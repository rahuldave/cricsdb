...
| `result` | `won` / `lost` / `tied` | Match-outcome filter from the path team's POV. `won` = path team is `outcome_winner`; `lost` = another team won; `tied` = `outcome_winner IS NULL` (collapses true ties + no-results — T20 ties go to super-over and that winner becomes `outcome_winner`, so NULL is almost exclusively rain-abandoned). Only meaningful when a path `:team` is bound — non-team endpoints silently ignore. Spec: `internal_docs/spec-splits-mosaic.md`. | `result=won` |
| `toss_outcome` | `won` / `lost` | Toss-outcome filter from the path team's POV. `won` = `toss_winner = :team`; `lost` = `toss_winner != :team AND toss_winner IS NOT NULL`. Spec: `internal_docs/spec-splits-mosaic.md`. | `toss_outcome=won` |
